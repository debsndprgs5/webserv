...
int pipexec(char **arg, char **envp, int *ret, int stdoutFd, int stdinFd, pid_t &childPid)
{
	...
	else if (pid == 0)
	{
		...
		if (stdinFd != -1) {
			...
			// We can close stdinFd after dup2
			close(stdinFd);
		}
							   ...
							   const char *path, int stdoutFd, int stdinFd, std::string uri)
{
	...
	if (!pipexec((char **)arg, envp_arr, ret, stdoutFd, stdinFd, childPid))
		std::cout << "Error : Something went wrong with pipex" << std::endl;

	// We stock childPid in client for poll's processing
...
void Methods::startCgiAsync(int reqtype, std::string cgiArg)
{
	...
	if (reqtype == 1) { // POST
		...
		// For GET, we do not redirect STDIN but -1
		stdinPipe[0] = -1;
		stdinPipe[1] = -1;
	}

	// Calling cgi_php_handler with stdoutPipe[1] and stdinPipe[0] (or -1 for Get)
	cgi_php_handler(&_ret, _cgiName.c_str(), &cgiArg, reqtype, _cgiPath.c_str(),
					stdoutPipe[1], (reqtype == 1 ? stdinPipe[0] : -1), _parsedRequest.uri);
	close(stdoutPipe[1]); // We close write side of stdout in parent


	if (reqtype == 1) {
		// For POST, write cgiArg's content in stdin pipe
		...
		close(stdinPipe[1]); // Close STDIN write's pipe in parent
	}

	_client->setRet(200);

	// Put stdoutPipe[0] in non-blocking mode
	int flags = fcntl(stdoutPipe[0], F_GETFL, 0);
	fcntl(stdoutPipe[0], F_SETFL, flags | O_NONBLOCK);

	// Stock CGI pipe's fd (stdoutPipe[0]) in client
	_client->setCgiPipe(stdoutPipe[0]);

	// Add CGI stdout fd to fdarray (addarray in this case, not same fdarray as processing)
	struct pollfd cgiFd;
